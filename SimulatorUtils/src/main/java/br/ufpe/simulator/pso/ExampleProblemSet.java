...
public class ExampleProblemSet implements IProblemSet {

	private List<ProblemRange> problemRanges;
	...
	public List<ProblemRange> getProblemRanges() {
		if (problemRanges == null) {
			problemRanges = new ArrayList<ProblemRange>();
			ProblemRange xProblemRange = new ProblemRange();
			xProblemRange.setHigh(4);
			xProblemRange.setLow(1);
			xProblemRange.setVelocityLow(-1);
			xProblemRange.setVelocityHigh(1);
			problemRanges.add(xProblemRange);
			ProblemRange yProblemRange = new ProblemRange();
			yProblemRange.setHigh(1);
			yProblemRange.setLow(-1);
			yProblemRange.setVelocityLow(-1);
			yProblemRange.setVelocityHigh(1);
			problemRanges.add(yProblemRange);
		}
